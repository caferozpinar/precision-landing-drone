...
camera_threshold = (0, 0, 120), (179, 120, 255)
...
class precisionLand():
    ...
    def Find(self):
        success_2, centers = self.camera.detect()
        success, self.Tshape = self.TFinder.findShape(centers)
        if success == 0:return 0
        if success == 1:return 1
        if success == 2:return 2
    def headPointer(self):
        olamaz = self.vehicle.trackCoordinates(self.Tshape, self.camera.resolution)
        if olamaz == 2 : return 2
        if olamaz == 0 : return 0
        if olamaz == 1 : return 1
    def waitMode(self):
...
if __name__ == "__main__":
    ...
    while True:
        ...
        #print("pointer = " + str(pointer_missing) + "\nshape_detect = " + str(shape_not_detected) + "\nneedpid = " + str(need_pid))
        if pointer_missing:
            led.off()
